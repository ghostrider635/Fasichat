<?php require __DIR__ . '/_bootstrap.php'; requireDashboardRoles(['Apparitaire']); ?>
<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
...
function publishAnnonce(){
  ...
  if(!t||!c){alert('Veuillez remplir le titre et le contenu.');return;}
  addAnnonce(t,c,cat||'📢 Information');
...
function cleanCategoryBackend(value){return value.replace(/[^\p{L}\p{N}\s-]/gu,'').trim()||'Information';}
publishFromModal=async function(){
  const t=document.getElementById('mTitle').value.trim();
  const c=document.getElementById('mContent').value.trim();
  if(!t||!c){alert('Veuillez remplir le titre et le contenu.');return;}
  const cat=document.getElementById('mCat').value;
  const data=new FormData();data.append('titre',t);data.append('contenu',c);data.append('categorie',cleanCategoryBackend(cat));
  try{await apiPostBackend('valve_create',data);}catch(error){alert(error.message||'Erreur pendant la publication.');return;}
  addAnnonce(t,c,cat);closeModal();document.getElementById('mTitle').value='';document.getElementById('mContent').value='';
};
publishAnnonce=async function(){
  const t=document.getElementById('compTitle').value.trim();
  const c=document.getElementById('compContent').value.trim();
  const cat=document.getElementById('compCat').value;
  if(!t||!c){alert('Veuillez remplir le titre et le contenu.');return;}
  const data=new FormData();data.append('titre',t);data.append('contenu',c);data.append('categorie',cleanCategoryBackend(cat));
  try{await apiPostBackend('valve_create',data);}catch(error){alert(error.message||'Erreur pendant la publication.');return;}
  addAnnonce(t,c,cat||'Information');document.getElementById('compTitle').value='';document.getElementById('compContent').value='';document.getElementById('compCat').value='';
};
