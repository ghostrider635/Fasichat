...
class DashboardService
{
    ...
    {
        $rows = $this->fetchAllSafe($sql, $params);
        return $rows[0] ?? null;
    }

    private function fetchValueSafe(string $sql, array $params = [], mixed $default = 0): mixed
    {
        $pdo = $this->db();
        if (!$pdo) {
            return $default;
    ...
    {
        $userId = (int)(\App\Services\SessionManager::get('user_id') ?? 0);
        return $userId > 0 ? $userId : 0;
    }

    private function initials(?array $user, string $fallback = 'FC'): string
    {
        if (!$user) {
            return $fallback;
        }
    ...
    public function pageData(): array
    {
        ...
        $user = $userId > 0 ? $this->fetchOneSafe(
            'SELECT u.id, u.nom, u.prenom, u.email, r.nom AS role_nom FROM utilisateurs u JOIN roles r ON r.id = u.role_id WHERE u.id = :id',
            ['id' => $userId]
        ) : null;

        $stats = [
            'users' => (int)$this->fetchValueSafe('SELECT COUNT(*) FROM utilisateurs'),
            'teachers' => (int)$this->fetchValueSafe("SELECT COUNT(*) FROM utilisateurs u JOIN roles r ON r.id = u.role_id WHERE r.nom IN ('Enseignant', 'Assistant')"),
            'students' => (int)$this->fetchValueSafe("SELECT COUNT(*) FROM utilisateurs u JOIN roles r ON r.id = u.role_id WHERE r.nom = 'Etudiant'"),
            'courses' => (int)$this->fetchValueSafe('SELECT COUNT(*) FROM cours'),
            'promotions' => (int)$this->fetchValueSafe('SELECT COUNT(*) FROM promotions'),
            'annonces' => (int)$this->fetchValueSafe('SELECT COUNT(*) FROM annonces_valve'),
            'convocations' => (int)$this->fetchValueSafe('SELECT COUNT(*) FROM convocations'),
            'messages' => (int)$this->fetchValueSafe('SELECT COUNT(*) FROM messages'),
        ];

        return [
            'user' => $user,
            'userInitials' => $this->initials($user),
            'csrfToken' => SecurityService::csrfToken(),
            'stats' => $stats,
            'users' => $this->fetchAllSafe(
            ...
            'promotionsList' => $this->fetchAllSafe(
                'SELECT p.id,
                        p.nom,
                        COUNT(DISTINCT ep.etudiant_id) AS student_count,
            ...
            'convocations' => $this->fetchAllSafe(
            ...
            'privateContacts' => $user ? $this->fetchAllSafe(
