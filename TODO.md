...
- [x] Mettre à jour `public/valve.php` si nécessaire pour correspondre au rendu JS (containers, classes, modals).



## Étape 2 — Messagerie (upload média)
- [x] Vérifier que `FileController` permet déjà l’upload général.
- [x] Étendre `MessageController::send()` pour accepter un upload (fichier/image/vidéo/voice) et le lier à un message via `messages.fichier_id`.
- [x] Mettre à jour le front `public/conversation.php` pour envoyer aussi un média (inputs file + preview) et un message texte optionnel.
- [x] Rendre les messages avec preview (audio player / image / video / lien fichier) dans la liste.
